...
os.environ["VLLM_USE_PINNED_MEMORY"] = "1"
...
def main():
    ...
    eagle_model_name = "leptonai/EAGLE-Llama-3.2-3B-Instruct"
    ...
    samp = SamplingParams(temperature=0.0, max_tokens=max_new_tokens, logprobs=None)

    # override global generate()
    # def generate(model_unused, input_ids, past_unused, max_new_tokens):
    #     prompt = tokenizer.decode(input_ids[0], skip_special_tokens=True)
    #     out = model.generate([prompt], samp)[0]
    #     print(out.metrics)
    #     return torch.tensor(out.outputs[0].token_ids, device=device).unsqueeze(0)
    ...
    model.forward = torch.compile(
        model.forward, mode="reduce-overhead", dynamic=False, fullgraph=True
    )
    ...
    for i in tqdm(range(5), desc="Warm Up..."):
        ...
        # )
        _ = model.generate([warmup_prompt], samp)
    ...
    for _ in tqdm(range(10), desc="Test Inference"):
        ...
        #     pad_token_id=tokenizer.eos_token_id,
        # )
        generated = model.generate([warmup_prompt], samp)
    ...
    # gc.collect()
    # torch.cuda.empty_cache()
    # torch.cuda.ipc_collect()  # full release
